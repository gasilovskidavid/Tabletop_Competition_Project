...
for (page_number in 1:19) {
  ...
  for (card in product_cards) {
    ...
    name <- card %>%
      html_element(xpath = "./div[2]/p[1]/a") %>%
      html_text2()
    price_text <- card %>%
      html_element(xpath = "./div[3]/p[1]/span") %>%
      html_text2()
    reviews <- card %>%
      html_element(xpath = "./div[2]/div[2]/div[1]/div/div[1]/span[2]") %>%
      html_text2()
    num_players <- card %>%
      html_element(xpath = "./div[2]/ul/li[3]/span") %>%
      html_text2()

    stock_check <- card %>%
      html_element(xpath = "./div[3]/div/div/div/a[1]") %>%
      html_attr("disabled")
